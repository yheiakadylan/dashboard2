...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const popupTabButtons = Array.from(document.querySelectorAll<HTMLButtonElement>('[data-popup-tab]'));
    const popupPanels = Array.from(document.querySelectorAll<HTMLElement>('[data-popup-panel]'));

    type PopupTab = 'connection' | 'reviews';

    const activatePopupTab = (tab: PopupTab, persist = true) => {
        popupTabButtons.forEach(button => button.classList.toggle('active', button.dataset.popupTab === tab));
        popupPanels.forEach(panel => panel.classList.toggle('active', panel.dataset.popupPanel === tab));
        if (persist) void chrome.storage.local.set({ popup_active_tab: tab });
    ...
    popupTabButtons.forEach(button => {
        button.addEventListener('click', () => {
            const tab = button.dataset.popupTab as PopupTab;
            if (['connection', 'reviews'].includes(tab)) activatePopupTab(tab);
        });
    });

    // Load existing saved values
    const data = (await chrome.storage.local.get(['teamId', 'account', 'dbEmail', 'dbPassword', 'popup_active_tab'])) as { [key: string]: string };
    ...
    const savedPopupTab = data.popup_active_tab as PopupTab;
    const initialPopupTab = ['connection', 'reviews'].includes(savedPopupTab)
        ? savedPopupTab
        : data.teamId && data.account && data.dbEmail && data.dbPassword ? 'reviews' : 'connection';
    activatePopupTab(initialPopupTab, false);
    ...
    const formatStatusDate = (value?: string, timeZone = 'Asia/Ho_Chi_Minh') => {
        if (!value) return '-';
    ...
    const renderReviewSyncStatus = (status?: ReviewSyncStatus) => {
        ...
        reviewSyncLastResult.textContent = typeof status?.lastFetched === 'number' || typeof status?.lastSaved === 'number'
            ...
            : '-';
        reviewSyncNextRun.textContent = state !== 'running' && status?.nextRunAt
            ? formatStatusDate(status.nextRunAt)
            : '-';
