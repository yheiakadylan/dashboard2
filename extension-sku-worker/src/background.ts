...
const etsyReviewSync = createEtsyReviewSync({
    ...
    ensureAuth,
    sleep,
    reportError: status => reportCrawlerError('review_error', status),
    markEtsyLoggedOut: () => { isEtsyLoggedOut = true; },
    markEtsyLoggedIn: () => { isEtsyLoggedOut = false; },
    setRateLimitUntil: (timestamp: number) => { rateLimitUntil = timestamp; },
    getRateLimitUntil: () => rateLimitUntil,
});
    ...
    try {
        ...
        if (!snap.empty) {
            const accDoc = snap.docs[0];
             const status = isEtsyLoggedOut
                 ? 'error'
                 : (isProcessing || isReviewSyncProcessing ? 'processing' : 'idle');
...
async function processRemoteWorkerCommand(teamId: string, command: any): Promise<{ started?: boolean; skipped?: boolean }> {
    ...
    if (command.target !== 'reviews') return { skipped: true };
...
async function reportCrawlerError(type: 'review_error', status: any): Promise<void> {
    ...
    const occurredAt = String(status.lastFinishedAt || status.updatedAt || new Date().toISOString());
    const payload = {
        action: 'worker-alert',
        teamId,
        ...
        type,
        occurredAt,
        currentAction: String(status.currentAction || ''),
        error,
        eventKey: `${occurredAt}|${error}`,
    };
    try {
        await callExtensionApi(payload);
    } catch {
        await sleep(1000);
        await callExtensionApi(payload);
    }
...
async function claimRemoteCommandViaApi(teamId: string, commandId: string, target: 'reviews' | 'health'): Promise<any | null> {
    ...
    const data = await callExtensionApi({
        ...
        target,
        commandId,
        workerAccount: String(storage.account || ''),
    });
