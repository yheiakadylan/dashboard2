...
window.addEventListener('message', (event) => {
  ...
  if (!msg.type.startsWith('NHMEDIA_CMD_REVIEWS_')) return;

  const requestId = msg.requestId || null;

  if (msg.type === 'NHMEDIA_CMD_REVIEWS_CRAWL_NOW') {
    chrome.runtime.sendMessage(
      { type: 'CRAWL_RECENT_REVIEWS_25' },
      (response) => {
        response = normalizeRuntimeResponse(response);
        postResult('NHMEDIA_RESULT_REVIEWS_CRAWL_NOW', requestId, response, {
  ...
  if (msg.type === 'NHMEDIA_CMD_REVIEWS_RUN_SYNC') {
    chrome.runtime.sendMessage(
      { type: 'RUN_ETSY_REVIEW_SYNC' },
      (response) => {
        response = normalizeRuntimeResponse(response);
        postResult('NHMEDIA_RESULT_REVIEWS_RUN_SYNC', requestId, response, {
          started: response?.started ?? false,
          fetched: response?.fetched ?? 0,
          saved: response?.saved ?? 0
        });
  ...
  if (msg.type === 'NHMEDIA_CMD_REVIEWS_SET_CRON_HOURS') {
    ...
    chrome.storage.local.set({ etsy_review_sync_hours: hours }, () => {
      chrome.runtime.sendMessage(
        ...
        (response) => {
          response = normalizeRuntimeResponse(response);
          postResult('NHMEDIA_RESULT_REVIEWS_SET_CRON_HOURS', requestId, response, {
  ...
  if (msg.type === 'NHMEDIA_CMD_REVIEWS_GET_STATUS') {
    chrome.storage.local.get(['etsy_review_sync_status', 'etsy_review_sync_hours'], (result) => {
      window.postMessage({
        type: 'NHMEDIA_RESULT_REVIEWS_GET_STATUS',
        requestId,
        success: true,
...
function readEtsyReviewShops(teamId: string): any[] {
  const accountsKey = `nhmedia_accounts_${teamId}`;
...
setInterval(() => {
  const configEl = document.getElementById('nhmedia-health-config');
  if (!configEl) return;
