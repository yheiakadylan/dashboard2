...
export interface EtsyReviewSyncDeps extends EtsyReviewApiDeps {
    ...
    reportError?: (status: EtsyReviewSyncStatus) => Promise<void>;
}
...
async function persistReviewSyncStatus(update: Partial<EtsyReviewSyncStatus>): Promise<EtsyReviewSyncStatus> {
...
export function createEtsyReviewSync(deps: EtsyReviewSyncDeps) {
    const updateReviewSyncStatus = async (update: Partial<EtsyReviewSyncStatus>): Promise<void> => {
        const status = await persistReviewSyncStatus(update);
        if (update.state !== 'error' || !status.lastError) return;
        try {
            await deps.reportError?.(status);
        } catch (error) {
            console.warn('[Reviews] Failed to report crawler error:', error);
        }
    };
