...
function syncConfigToPage() {
    chrome.storage.local.get(['config'], (result) => {
        if (result?.config) {
            let div = document.getElementById('nhmedia-health-config');
            if (!div) {
                div = document.createElement('div');
                div.id = 'nhmedia-health-config';
...
window.addEventListener('message', (event) => {
    ...
    if (!msg.type.startsWith('NHMEDIA_CMD_HEALTH_')) return;

    const requestId = msg.requestId || null;

    if (msg.type === 'NHMEDIA_CMD_HEALTH_CHECK_RUN') {
        chrome.runtime.sendMessage(
            { type: 'TRIGGER_SHOP_HEALTH_CHECK', force: Boolean(msg.force) },
            (response) => {
                response = normalizeRuntimeResponse(response);
                window.postMessage({
                    type: 'NHMEDIA_RESULT_HEALTH_CHECK_RUN',
                    requestId,
    ...
    if (msg.type === 'NHMEDIA_CMD_HEALTH_CHECK_STOP') {
        chrome.runtime.sendMessage(
            { type: 'STOP_SHOP_HEALTH_CHECK' },
            (response) => {
                response = normalizeRuntimeResponse(response);
                window.postMessage({
                    type: 'NHMEDIA_RESULT_HEALTH_CHECK_STOP',
                    requestId,
                    success: response?.success ?? false
                }, '*');
            }
    ...
    if (msg.type === 'NHMEDIA_CMD_HEALTH_CHECK_STATUS') {
        chrome.runtime.sendMessage(
            { type: 'GET_SHOP_HEALTH_STATUS' },
            (response) => {
                response = normalizeRuntimeResponse(response);
                window.postMessage({
                    type: 'NHMEDIA_RESULT_HEALTH_CHECK_STATUS',
                    requestId,
                    success: response?.success ?? false,
