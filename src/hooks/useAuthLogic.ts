      ...
      if (currentUser) {
        try {
          ...
          if (!roleDoc.exists()) {
            ...
            const raw = roleDoc.data() as any;

            // Normalize permissions: support both nested map AND
            // literal dot-notation field names (e.g. "permissions.canAddDesign")
            // that Firestore Console sometimes creates when fields are added manually.
            ...
            Object.keys(raw).forEach((key) => {
              if (key.startsWith("permissions.")) {
                const subKey = key.slice("permissions.".length);
                permissions[subKey] = raw[key];
              }
            });

            const profile: UserProfile = { ...raw, permissions };
