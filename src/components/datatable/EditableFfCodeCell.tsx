...
interface EditableFfCodeCellProps {
    value: string | null | undefined;
    recordId: string;
    onUpdateFfCode?: (recordId: string, newFfCode: string) => Promise<void>;
}

const EditableFfCodeCell: React.FC<EditableFfCodeCellProps> = ({ value, recordId, onUpdateFfCode }) => {
    const [isEditing, setIsEditing] = useState(false);
    const [editValue, setEditValue] = useState(value || '');
    const [isSaving, setIsSaving] = useState(false);

    const handleSave = async () => {
        if (!onUpdateFfCode) return;
        setIsSaving(true);
        try {
            ...
            await onUpdateFfCode(recordId, trimmedValue);
            setIsEditing(false);
        } catch {
            setIsEditing(true);
        } finally {
    ...
    return (
        <div 
            ...
            title="Double click to edit FF Code manually"
        >
...
export default EditableFfCodeCell;
