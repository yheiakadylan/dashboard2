...
import { LayoutDashboard, Users, ChevronDown, Check, Download, LogOut, Settings } from 'lucide-react';
...
const NotificationCenter = React.lazy(() => import('../../features/notifications/components/NotificationCenter'));
...
const Header: React.FC = () => {
  ...
  const {
    ...
    allowedAccounts, // Operations shop permissions remain user-specific
  ...
  const supportsScopedOperations = true;
  const canManagePodTeams = role === 'owner'
    || permissions.canManageSettings === true
    || ['ADMIN', 'MANAGER'].includes(String(sharedRole || '').toUpperCase());
  const canFilterPerformanceByPodTeam = false;
  const {
    selectedAccountId,
  ...
  const userProfile = useMemo(() => teamId ? {
    teamId,
    role,
    permissions,
    allowedAccounts,
    email: user?.email
  } : null, [allowedAccounts, permissions, role, teamId, user?.email]);
  ...
  const recordsMissingSku = useMemo(() => {
    return visibleOrderRecords.filter(recordNeedsSkuFetch);
  }, [visibleOrderRecords]);
  ...
  const triggerBulkSyncSkuToTasks = async () => {
    ...
    try {
      ...
      if (!idToken) {
        addNotification('Please sign in again to sync orders.', 'error');
        setIsApiLoading(false);
        return;
      }
      ...
      if (res.ok) {
        addNotification(`Success: Created ${data.createdCount ?? 0} missing tasks`, 'success');
      } else {
  ...
  const triggerBulkFetchSku = async () => {
    ...
    try {
      ...
      const idToken = await user?.getIdToken();
      if (!idToken) {
        addNotification('Please sign in again to fetch SKUs.', 'error');
        setIsApiLoading(false);
        return;
      }
    ...
    <header className="glass-base border-b border-gray-200 dark:border-gray-700 sticky top-0 z-30 transition-all duration-200">
      ...
      <div className="px-4 h-16 flex items-center justify-between max-w-[1920px] mx-auto">
        ...
        <div className="flex items-center gap-3 min-w-0">
          {((canManagePodTeams && supportsScopedOperations) || canFilterPerformanceByPodTeam) && (
                  ...
                  <div className="max-h-[300px] overflow-y-auto space-y-0.5 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-700">
                    {boards.map(board => (
                      <button
                        key={board.uid}
                        onClick={() => {
                          setSelectedBoardId(board.uid || null);
                          setSelectedAccountId('all');
                          setIsBoardDropdownOpen(false);
                        }}
                      ...
                      >
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="font-medium truncate">{board.displayName || 'POD Team'}</span>
                          <span className="text-xs text-gray-500 truncate">
                            {board.description || `${board.allowedAccounts.length} shops`}
                          </span>
                        </div>
        ...
        {!isMobile && <div className="hidden md:flex items-center gap-3">
          {/* Collapsible Search */}
          {supportsScopedOperations && <div className={`relative flex items-center transition-all duration-300 ${isSearchExpanded ? 'w-48 lg:w-64' : 'w-10'}`}>
            {!isSearchExpanded ? (
              // Search Icon Button
              <button
          ...
          {supportsScopedOperations && <CustomSelect
          ...
          {canInstall && (
            <button
              ...
              className="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-blue-600 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-blue-400"
              title="Cài ứng dụng"
            >
                ...
                <div className="py-1">
                  {canInstall && (
                    <button
                      ...
                      className="flex w-full items-center px-4 py-2 text-left text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700/50"
                    >
        ...
        {isMobile && <div className="flex md:hidden items-center gap-1">
      ...
      {isMobile && <div className={`md:hidden transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'max-h-screen opacity-100 border-t border-gray-200 dark:border-gray-700 shadow-xl overflow-visible' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="p-4 bg-white dark:bg-gray-800 space-y-5">
          ...
          {/* Mobile Search */}
          {supportsScopedOperations && <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
          ...
          <div className="space-y-4">
            ...
            <div className={`grid gap-3 ${supportsScopedOperations ? 'grid-cols-2' : 'grid-cols-1'}`}>
              {supportsScopedOperations && <div>
          ...
          <div className="pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              {canInstall && (
                <button
                  onClick={() => void installApp()}
                  className="rounded-md p-2 text-gray-600 transition-colors hover:bg-gray-100 hover:text-blue-600 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-blue-400"
                  title="Cài ứng dụng"
                >
