...
const ReviewsTab: React.FC = () => {
    ...
    const reviewDateFormatter = useMemo(() => new Intl.DateTimeFormat('en-US', {
        timeZone,
        year: 'numeric',
        month: 'short',
        day: '2-digit',
    ...
    const paginatedReviews = useMemo(() => filteredReviews.slice(
    ...
    useEffect(() => {
        setCurrentPage(0);
    }, [reviewRatingFilter, selectedProductFilter, selectedShopKeys, showBadReviewsOnly, showImagesOnly]);

    useEffect(() => {
        setCurrentPage(page => Math.min(page, Math.max(0, totalReviewPages - 1)));
    }, [totalReviewPages]);
    ...
    const handleHighlightClick = (review: EtsyReview) => {
        ...
        const reviewIndex = filteredReviews.findIndex(item => getReviewKey(item) === key);
        if (reviewIndex >= 0) setCurrentPage(Math.floor(reviewIndex / REVIEWS_PER_PAGE));
        ...
        <div className="h-full bg-gray-50 dark:bg-gray-900 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none'] relative">
            <div className="p-2 md:p-6">
                ...
                <div className="flex flex-col gap-4 pb-20 max-w-7xl mx-auto">
                    {filteredReviews.length > 0 ? (
                        paginatedReviews.map((review) => {
                            ...
                            const dateDisplay = reviewDateFormatter.format(new Date(review.create_date));
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-start gap-3">
                                                    {review.listing_image?.url_75x75 ? (
                                                        <img src={review.listing_image.url_75x75} alt={review.listing_title} loading="lazy" className="w-[75px] h-[75px] shrink-0 object-cover rounded cursor-pointer hover:opacity-80" onClick={() => setPreviewImage(review.listing_image?.url_fullxfull || review.listing_image?.url_75x75 || null)} />
                                                    ) : (
                ...
                <div className="mx-auto max-w-7xl pb-20">
                    <Pagination currentPage={currentPage} totalPages={totalReviewPages} onPageChange={setCurrentPage} />
                </div>
            </div>
