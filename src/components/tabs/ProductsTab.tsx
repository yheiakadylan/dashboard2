...
import { OrderItem, ProcessedData, Record, TopProduct } from '../../types';
...
const getCategoryFilter = (query: string) => {
    const normalized = query.trim();
    return normalized.toLowerCase().startsWith(CATEGORY_FILTER_PREFIX)
        ? normalized.slice(CATEGORY_FILTER_PREFIX.length).trim().toLowerCase()
        : '';
};
...
const ProductsTab: React.FC<ProductsTabProps> = ({ processedData }) => {
    ...
    useEffect(() => {
        ...
        const fetchUsers = async () => {
            try {
                const { fetchOperationUsers } = await import('../../services/reportService');
                const users = await fetchOperationUsers();
                const mapping: { [empID: string]: string } = {};
                users.forEach(operationUser => {
                    const displayName = operationUser.displayName || operationUser.fullName;
                    if (operationUser.empID && displayName) {
                        mapping[String(operationUser.empID).trim()] = displayName;
                    }
                });
                if (!cancelled) setUserDisplayNames(mapping);
            } catch (e) {
    ...
    const handleCategoryClick = useCallback((category: TopProduct) => {
        setViewMode('product');
        setSearchTerm(`${CATEGORY_FILTER_PREFIX}${category.name}`);
    ...
    const filteredProducts = useMemo(() => {
        ...
        const categoryFilter = getCategoryFilter(searchTerm);
        if (categoryFilter) return rows.filter(row => String(row[3] || '').trim().toLowerCase() === categoryFilter);
        if (searchTerm) rows = rows.filter(row => filterByAdvancedSearch(`${row[1]} ${row[2]} ${row[3]} ${row[4]}`, searchTerm));
        return rows;
    ...
    const filteredVariants = useMemo(() => {
        ...
        const categoryFilter = getCategoryFilter(searchTerm);
        if (categoryFilter) return rows.filter(row => String(row[0] || '').trim().toLowerCase() === categoryFilter);
        if (searchTerm) rows = rows.filter(row => filterByAdvancedSearch(`${row[0]} ${row[1]}`, searchTerm));
        return rows;
    ...
    const handleExportInventory = useCallback(async () => {
        ...
        try {
            ...
            await exportInventoryToExcel(
                { ...processedData.products, rows: filteredProducts },
                { ...processedData.variants, rows: filteredVariants },
                true, // includeImages
                globalUsdMode,
                exchangeRates,
    ...
    }, [processedData.products, processedData.variants, filteredProducts, filteredVariants, globalUsdMode, exchangeRates, addNotification]);
