...
const Toast: React.FC<ToastProps> = ({ id, message, type, onClose }) => {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => onClose(id), remainingTimeRef.current);
    return () => window.clearTimeout(timeoutId);
  }, [id, isPaused, onClose]);

  const pauseTimer = () => {
    if (isPaused) return;
    remainingTimeRef.current = Math.max(0, remainingTimeRef.current - (Date.now() - startedAtRef.current));
    setIsPaused(true);
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/20 rounded-b-lg overflow-hidden">
        <div
          className={`h-full origin-left ${config.progressColor} animate-toast-progress`}
