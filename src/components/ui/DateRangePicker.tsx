...
const presets = [
    ...
    { label: 'This quarter', getRange: (today: Date) => ({ from: getQuarterStart(today), to: getQuarterEnd(today) }) },
    { label: 'Last quarter', getRange: (today: Date) => { const currentStart = getQuarterStart(today); const f = new Date(Date.UTC(currentStart.getUTCFullYear(), currentStart.getUTCMonth() - 3, 1)); const t = new Date(Date.UTC(currentStart.getUTCFullYear(), currentStart.getUTCMonth(), 0)); return { from: f, to: t }; } },
...
const DateRangePicker: React.FC<DateRangePickerProps> = ({ align = 'left', value, onChange, timeZone: timeZoneOverride }) => {
    ...
    const [desktopPopupStyle, setDesktopPopupStyle] = useState<React.CSSProperties>({});
    ...
    }, [selectedRange.from, selectedRange.to, activeTimeZone]);
    ...
    useEffect(() => {
        ...
        const updatePopupPosition = () => {
            ...
            const popupWidth = Math.min(720, window.innerWidth - viewportPadding * 2);
            const preferredLeft = align === 'right' ? triggerRect.right - popupWidth : triggerRect.left;
            const left = Math.min(
                Math.max(viewportPadding, preferredLeft),
                window.innerWidth - popupWidth - viewportPadding
            );
            const preferredHeight = Math.min(500, window.innerHeight - viewportPadding * 2);
            const spaceBelow = window.innerHeight - triggerRect.bottom - viewportPadding;
            const spaceAbove = triggerRect.top - viewportPadding;
            const opensAbove = spaceBelow < preferredHeight && spaceAbove > spaceBelow;
            const top = opensAbove
                ? Math.max(viewportPadding, triggerRect.top - preferredHeight - 8)
                : triggerRect.bottom + 8;
            const availableHeight = opensAbove
                ? triggerRect.top - top - 8
    ...
    // Responsive classes
    const containerClass = `font-sans bg-white dark:bg-gray-800 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row overflow-hidden ${isMobile ? 'w-full max-w-sm' : 'w-full'}`;

    const sidebarClass = `w-full md:w-44 p-2 border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700 overflow-y-auto ${isMobile ? 'max-h-32 grid grid-cols-2 gap-1' : 'max-h-[430px]'}`;
    ...
    return (
        <div ref={wrapperRef} className="relative w-full md:w-44">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 flex items-center">
                ...
                <span className="text-sm ml-2 flex-grow text-left truncate" title={buttonText}>{buttonText}</span>
            ...
            {isOpen && (
                isMobile
                    ...
                    : createPortal(
                        <div ref={popupRef} className="fixed z-[120] overflow-y-auto overscroll-contain" style={desktopPopupStyle}>
                            {popupContent}
                        </div>,
                        document.body
                    )
