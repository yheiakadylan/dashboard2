...
import { Record, Account, ProcessedData, ManualCost, PODTeam, Category, EtsyReview, type Tab } from '../types';
import {
...
interface DashboardContextType {
  ...
  // POD Team selection for Operations and Performance scopes
  boards: PODTeam[];
...
const useStableJsonValue = <T,>(value: T): readonly [T, string] => {
  ...
  return useMemo(() => {
    ...
    return [stableRef.current.value, stableRef.current.hash] as const;
  }, [value]);
};
...
export const DashboardProvider: React.FC<DashboardProviderProps> = ({
  ...
  // Board Logic
  const [boards, setBoards] = useState<PODTeam[]>([]);
  const [selectedBoardId, setSelectedBoardId] = useState<string | null>(null);
  const canUsePodTeams = role === 'owner'
  ...
  const refreshBoards = React.useCallback(async () => {
    if (canUsePodTeams && teamId) {
      try {
        const fetchedTeams = await getPODTeams(teamId);
        setBoards(fetchedTeams);
      } catch (error) {
        console.error('Failed to refresh POD teams:', error);
        addNotification('Không thể tải cấu hình POD team.', 'error');
      }
    } else {
      setBoards([]);
    }
  }, [addNotification, canUsePodTeams, teamId]);
  ...
  useEffect(() => {
    if (selectedBoardId && !boards.some(board => board.uid === selectedBoardId)) {
  ...
  const [stableProcessingAccounts, processingAccountsHash] = useStableJsonValue(processingAccounts);
  ...
  useEffect(() => {
    ...
    const relevantManualCosts = scopeNeedsManualCosts(processingScope) ? manualCosts : null;
    const relevantRates = scopeNeedsExchangeRates(processingScope) ? exchangeRates : null;
  ...
  }, [activeTab, filteredRecords, scopedPreviousPeriodRecords, processingAccountsHash, filterDateRange, timeZone, role, permissions, manualCosts, exchangeRates, categories, visibleEtsyReviews, isFetchingNewRange, currentDataKey, clearWorkerSafetyTimeout]);
  ...
  }, [filteredRecords, scopedPreviousPeriodRecords, stableProcessingAccounts, filterDateRange, timeZone, role, permissions, manualCosts, exchangeRates, categories, visibleEtsyReviews]);
  ...
  const handleSaveAccounts = useCallback(async (updatedAccounts: Account[], explicitlyRemovedIds: string[] = []) => {
    ...
    try {
      ...
      if (newAccounts.length > 0) {
        ...
        const gmailAccounts = newAccounts.filter(acc => acc.provider === 'gmail');
        if (gmailAccounts.length > 0) void import('../services/emailService').then(({ setupGmailWatch }) => {
          gmailAccounts.forEach(acc => setupGmailWatch(teamId, acc).catch(console.error));
        });
  ...
  const handleExportWithOptions = useCallback(async (includeImages: boolean) => {
    ...
    if (filteredRecords.length === 0 && manualCosts.length === 0) {
      addNotification('No data to export', 'info');
      return;
    }
  ...
  }, [isExporting, filteredRecords.length, manualCosts.length, addNotification, timeZone, buildFullProcessedDataForExport, globalUsdMode, exchangeRates]);
  ...
  const accessContextValue = useMemo<DashboardAccessContextType>(() => ({
    user,
    displayName,
    employeeId,
    teamId,
  ...
  return (
    <DashboardAccessContext.Provider value={accessContextValue}>
...
export const useDashboardAccess = () => {
  const context = React.useContext(DashboardAccessContext);
  if (context === undefined) {
