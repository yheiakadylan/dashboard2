...
import { DEFAULT_TAB_ORDER } from "../constants";

const DEFAULT_TABS: Tab[] = DEFAULT_TAB_ORDER;
...
interface UIContextType {
  ...
  setSourceFilter: React.Dispatch<
    React.SetStateAction<"All" | "Ebay_Sales" | "Etsy_Sales">
  >;
  supportFilter: "All" | "Case" | "Help";
  setSupportFilter: React.Dispatch<
    React.SetStateAction<"All" | "Case" | "Help">
  >;
...
export const UIProvider: React.FC<{
  ...
  userUid?: string;
  teamId?: string;
}> = ({ children, userUid, teamId }) => {
  const { addNotification } = useNotification();

  // --- 1. Local Storage State ---
  const [activeTab, setActiveTabRaw] = useLocalStorage<Tab>(
    "activeTab",
    "Overview",
  );
  ...
  const prefKey =
    userUid && teamId
      ? `tabPreferences_${teamId}_${userUid}`
      : "tabPreferences_guest";
  const [tabPreferences, setTabPreferences] = useLocalStorage<{
    tabOrder: Tab[];
    hiddenTabs: Tab[];
  }>(prefKey, { tabOrder: DEFAULT_TABS, hiddenTabs: [] });
  ...
  const [tabOrder, setLocalTabOrder] = useState<Tab[]>(() => {
    ...
    const savedOrder = tabPreferences.tabOrder.filter((tab) =>
      validTabs.has(tab),
    );
  ...
  const [hiddenTabs, setHiddenTabs] = useState<Set<Tab>>(
    new Set(tabPreferences.hiddenTabs),
  );
  ...
  useEffect(() => {
    if (prevTimeZone.current !== timeZone) {
      ...
      if (
        filterDateRange.from === oldToday &&
        filterDateRange.to === oldToday
      ) {
        console.log(
          `[UIContext] Timezone changed: updating "Today" from ${oldToday} to ${newToday}`,
        );
  ...
  const [sourceFilter, setSourceFilter] = useState<
    "All" | "Ebay_Sales" | "Etsy_Sales"
  >("All");
  const [supportFilter, setSupportFilter] = useState<"All" | "Case" | "Help">(
    "All",
  );
