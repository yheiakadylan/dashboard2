...
import { sendLarkLoginNotification } from '../../notifications/services/notificationService';
...
const LoginNotificationHandler: React.FC<LoginNotificationHandlerProps> = ({ user, userProfile }) => {
    ...
    React.useEffect(() => {
        if (user && userProfile && !hasShownNotification.current) {
            ...
            sendLarkLoginNotification(user.email, userProfile.role, userProfile.teamId, userProfile.displayName || user.displayName);
