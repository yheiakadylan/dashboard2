...
const Auth: React.FC<{ authError?: string | null }> = ({ authError }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(String(err?.message || 'An unknown error occurred. Please try again.').replace('Firebase: ', ''));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      const message = String(err?.message || 'Unable to send password reset email.').replace('Firebase: ', '');
      setError(message);
    ...
    <div className="min-h-screen bg-gradient-mesh flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md glass-panel rounded-2xl shadow-2xl p-8 border border-white/20 relative z-10 backdrop-blur-xl">
        <div className="flex flex-col items-center justify-center mb-8">
          <div className="bg-white/20 p-4 rounded-full shadow-inner mb-4">
            ...
            >
              ...
              <path d="M4 15L9 9L14 13L20 8" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-sm" />
            </svg>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label htmlFor="identifier" className="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5 ml-1">
          ...
          <div className="pt-2">
            ...
            >
              {isLoading ? <Spinner size="md" color="text-white" /> : 'Sign In'}
            </button>
          </div>
        </form>
      </div>
