        ...
        list.forEach((n, index) => {
            ...
            if (n.type === 'LOGIN' && n.metadata?.login_info?.user_email) {
                ...
                if (groupedLogins.length > 0) {
                    const userNameDisp = n.metadata.login_info.user_name || userEmail;
                    // Create a "Grouped" notification object (virtual)
                    const groupedNotif: Notification = {
                        ...n,
                        title: `${groupedLogins.length + 1} Logins from ${userNameDisp}`,
                        content: `User ${userNameDisp} logged in ${groupedLogins.length + 1} times recently.`,
