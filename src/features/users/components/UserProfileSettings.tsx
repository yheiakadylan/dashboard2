...
import { updateAuthenticationProfile, uploadDashboardAvatar } from '../../../services/firebaseService';
import { saveImageToDB, getImageFromDB } from '../../../utils/indexedDB'; // Keep for cache/fallback if needed
import Spinner from '../../../components/ui/Spinner';
import { User as UserIcon, Camera, Save, Sparkles, Mail, Link as LinkIcon, ShieldCheck, Upload, Info, LockKeyhole, KeyRound, BadgeCheck, ArrowLeft, ChevronRight, AlertCircle, Globe, Laptop, Clock, Monitor } from 'lucide-react';

const UserProfileSettings: React.FC = () => {
    const { user, userProfile } = useAuthLogic();
    const { addNotification } = useNotification();
    const { refreshBoards } = useDashboardAccess();
    const managedDisplayName = userProfile?.displayName || userProfile?.fullName || user?.displayName || '';
    ...
    const browserInfo = userAgent.includes('Edg/') ? 'Microsoft Edge'
        : userAgent.includes('Chrome/') ? 'Google Chrome'
            : userAgent.includes('Firefox/') ? 'Mozilla Firefox'
                : userAgent.includes('Safari/') ? 'Apple Safari'
                    : 'Unknown Browser';
    const osInfo = userAgent.includes('Win') ? 'Windows'
        : userAgent.includes('Android') ? 'Android'
            : userAgent.includes('iOS') || /iPhone|iPad|iPod/.test(userAgent) ? 'iOS'
    ...
    useEffect(() => {
        ...
        fetch('https://api.ipify.org?format=json', { signal: controller.signal })
            ...
            .catch(() => {
                if (!controller.signal.aborted) setIpAddress('Unavailable');
            });
    ...
    useEffect(() => {
        ...
        if (user) {
            setDisplayName(managedDisplayName);
            setPhotoURL(user.photoURL || '');

            // Check for local avatar override
            getImageFromDB(user.uid).then((blob) => {
                if (!blob || cancelled) return;
                photoObjectUrlRef.current = URL.createObjectURL(blob);
                setPhotoURL(photoObjectUrlRef.current);
            }).catch(console.error);
        }
        return () => { cancelled = true; };
    }, [managedDisplayName, user]);
    ...
    const handleSaveProfile = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 2. Update the shared authentication profile.
            await updateAuthenticationProfile(user.uid, {
                    ...
                    <div className="relative z-10 animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col">
                        ...
                        <form onSubmit={handleSaveProfile} className="space-y-5 flex-1 flex flex-col">
                            ...
                            <div className="group">
                                ...
                                <div className={`relative flex items-center bg-gray-50 dark:bg-gray-900/50 border rounded-lg overflow-hidden transition-all duration-300 ${activeField === 'name' ? 'border-blue-500 ring-2 ring-blue-500/10' : 'border-gray-200 dark:border-gray-700'}`}>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-transparent px-3 py-2 outline-none text-sm text-gray-900 dark:text-gray-100"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex justify-end pt-2 gap-2 mt-auto">
                                <button
                                    type="button"
                                    onClick={() => { setDisplayName(managedDisplayName); setPhotoURL(user?.photoURL || ''); }}
